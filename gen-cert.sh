...
keytool \
	...
	-dname "CN=control-center, O=Home, OU=Home, L=Campinas, C=BR" \
	-validity 3650 \
	-keyalg RSA \
	-keysize 4096 \
...
keytool \
	...
	-dname "CN=application, O=Home, OU=Home, L=Campinas, C=BR" \
	-validity 3650 \
	-keyalg RSA \
	-keysize 4096 \
...
echo "=> Appplication akhq"

echo " => Generate the private keys (for akhq)"
keytool \
	-genkeypair \
	-alias akhq \
	-dname "CN=akhq, O=Home, OU=Home, L=Campinas, C=BR" \
	-validity 3650 \
	-keyalg RSA \
	-keysize 4096 \
	-keystore akhq.keystore.jks \
	-storetype jks \
	-keypass changeit \
	-storepass changeit > /dev/null 2>&1

echo " => Generate certificate for the client signed by ca (root -> ca -> akhq)"
keytool \
	-keystore akhq.keystore.jks \
	-storetype jks \
	-validity 3650 \
...
keytool \
	...
	-rfc \
	-infile akhq.csr \
	-outfile akhq.pem > /dev/null 2>&1

echo " => Import the client cert chain into akhq.keystore.jks"
keytool \
	-keystore akhq.keystore.jks \
	-storetype jks \
	-storepass changeit \
	...
	-alias akhq \
	-noprompt \
	-file akhq.pem > /dev/null 2>&1

echo " => Import the client cert chain into akhq.truststore.jks"
keytool \
	-keystore akhq.truststore.jks \
	-storetype jks \
	-storepass changeit \
	-importcert \
	-trustcacerts \
...
--from-file=./application.client.keystore.jks \
